...
class TripSpider(scrapy.Spider):
    ...
    #                     city = random.choice(all_cities-1)
    #                     city_id = city.get("id", "")
        ...
        if json_script:
            ...
            if match:
                try:
                    ...
                    print(f"Total Inbound Cities: {len(inbound_cities)}\n")

                    if inbound_cities:
                        # Fetch the 0th inbound city
                        city = inbound_cities[0]
                        city_id = city.get("id", "")
                        city_name = city.get("name", "Unknown City")

                        print(f"Selected 0th Inbound City: {city_name} (ID: {city_id})\n")
                        ...
                        print("No inbound cities found in the JSON data.\n")

                    if outbound_cities:
                        # Fetch the 0th outbound city
                        city = outbound_cities[0]
                        city_id = city.get("id", "")
                        city_name = city.get("name", "Unknown City")

                        print(f"Selected 0th Outbound City: {city_name} (ID: {city_id})\n")
                        ...
                        print("No outbound cities found in the JSON data.\n")
            ...
            if match:
                try:
                    ...
                    for hotel in hotels:
                        ...
                        item["hotel_id"] = hotel.get("hotelBasicInfo", {}).get("hotelId", "")
